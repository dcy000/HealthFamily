...
import com.gzq.lib_core.http.interceptor.BufferListener;
...
final class ObjectFactory {
    ...
    public static OkHttpClient getOkHttpClient(Context context, GlobalConfig globalConfig) {
        ...
        if (BuildConfig.DEBUG) {
            okhttpBuilder.addInterceptor(getLoggingInterceptor(globalConfig));
        }
        //添加动态变更BaseUrl的能力
    ...
    public static LoggingInterceptor getLoggingInterceptor(GlobalConfig globalConfig) {
        return new LoggingInterceptor.Builder()
                ...
                .enableMock(globalConfig.isCanMock(), 100, new BufferListener() {
                    @Override
                    public String getJsonResponse(Request request) throws IOException {
                        String segment = request.url().pathSegments().get(0);
                        return Okio.buffer(Okio.source(App.getApp().getAssets().open(String.format("mock/%s.json", segment)))).readUtf8();
                    }
                })
                .build();
    }
