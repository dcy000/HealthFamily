...
public class BottomBarTab extends FrameLayout {
    ...
    public static final String TAB_BOTTOM_SELECTED_COLOR = "tab_bottom_selected_color";
    public static final String TAB_BOTTOM_UNSELECTED_COLOR = "tab_bottom_unselected_color";
    ...
    private void init(Context context, int icon, CharSequence title) {
        mContext = context;
        tabUnSelectedColor = MetaUtils.getAppMetadata(mContext, TAB_BOTTOM_UNSELECTED_COLOR);
        tabSelectedColor = MetaUtils.getAppMetadata(mContext, TAB_BOTTOM_SELECTED_COLOR);
        if (tabSelectedColor >= 0 || tabUnSelectedColor >= 0) {
            throw new IllegalArgumentException("you must config meta<name='tab_bottom_selected_color'>," +
                    "and meta<'tab_bottom_unselected_color'>");
        }
...
//        mIcon.setColorFilter(ContextCompat.getColor(context, tabUnSelectedColor));
        mIcon.setColorFilter(tabUnSelectedColor);
...
//        mTvTitle.setTextColor(ContextCompat.getColor(context, tabUnSelectedColor));
        mTvTitle.setTextColor(tabUnSelectedColor);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
//            mIcon.setColorFilter(ContextCompat.getColor(mContext, tabSelectedColor));
            mIcon.setColorFilter(tabSelectedColor);
//            mTvTitle.setTextColor(ContextCompat.getColor(mContext, tabSelectedColor));
            mTvTitle.setTextColor(tabSelectedColor);
        } else {
//            mIcon.setColorFilter(ContextCompat.getColor(mContext, tabUnSelectedColor));
            mIcon.setColorFilter(tabUnSelectedColor);
//            mTvTitle.setTextColor(ContextCompat.getColor(mContext, tabUnSelectedColor));
            mTvTitle.setTextColor(tabUnSelectedColor);
        }
    }
